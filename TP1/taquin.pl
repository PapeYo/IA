...
Doit contenir au moins 4 predicats qui seront utilises par A*
   ...
   rule(Rule_Name, Rule_Cost, Before_State, After_State)   % regles applicables

   heuristique(Current_State, Hval)				           % calcul de l'heuristique

Les autres predicats sont specifiques au Taquin.
*/

%:- lib(listut).      % Laisser cette directive en commentaire si vous utilisez Swi-Prolog

% Sinon decommentez la ligne si vous utilisez ECLiPSe Prolog :
...
delete(N,X,[Y|L], [Y|R]) :-
   ...
   %*******************************************************************
	...
	% Definit la relation entre des coordonnees [Ligne, Colonne] et un element de la matrice
	/*
	Exemples
...
%   heuristique1(U,H,F).  % au debut on utilise l'heuristique 1
   ...
   % Suggestions : definir d'abord le predicat coordonnees(Piece,Etat,Lig,Col) qui associe a une piece presente dans Etat
   ...
   % On peut egalement comparer les pieces qui se trouvent aux memes coordonnees dans U et dans H et voir s'il sagit de la
   % meme piece.
