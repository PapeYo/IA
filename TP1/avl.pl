...
% put_flat(+Avl)                    O(N)
% put_90(+Avl)                      O(N)
% belongs(+Elem, +Avl)              O(log N)		%<<< appartenance d'un noeud a Q
...
left_rotate(avl(G,R,D,_), A_Apres) :-
	...
	% T1 @< T2 est vrai si T1 est lexicographiquement inferieur a T2.
...
insert(Elem, AVL, NEW_AVL) :-
	...
	% sinon le predicat echoue (en particulier si l'AVL est vide).
